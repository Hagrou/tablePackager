    ...
    def extract(self, table_name, package):
        ...
        for file in Path(self.pinballX_path).glob('**/*%s*' % table_name):
            if "Flyer Images\\Back" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_back" + file.suffix)
            elif "Flyer Images\\Front" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_front" + file.suffix)
            elif "Flyer Images\\Inside1" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_inside1" + file.suffix)
            elif "Flyer Images\\Inside2" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_inside2" + file.suffix)
            elif "Flyer Images\\Inside3" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_inside3" + file.suffix)
            elif "Flyer Images\\Inside4" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_inside4" + file.suffix)
            elif "Flyer Images\\Inside5" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_inside5" + file.suffix)
            elif "Flyer Images\\Inside6" in str(file.parent):
                package.add_file(file, 'media/GameInfo', dstFile=file.stem + ".flyer_inside6" + file.suffix)
                ...
                package.add_file(file, 'media/GameHelp')
